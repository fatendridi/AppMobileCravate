...
public  class ProduitAdapterCravateEnfant extends RecyclerView.Adapter <MyViewHolderCravateEnfant> {
    ArrayList<ProduitCravateEnfant> dataset;
    Activity act ;

    public ProduitAdapterCravateEnfant(ArrayList<ProduitCravateEnfant> DataModels, Activity act) {
    ...
    @Override
    public MyViewHolderCravateEnfant onCreateViewHolder(ViewGroup parent, int viewType ) {
        View view= LayoutInflater.from( act ).inflate( R.layout.item_posts_cravate_enfant , parent,false );
        return new MyViewHolderCravateEnfant( view);
    }

    @Override
    public void onBindViewHolder(MyViewHolderCravateEnfant holder, int position) {
