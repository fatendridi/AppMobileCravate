...
public  class ProduitAdapterCravateClassique extends RecyclerView.Adapter <MyViewHolderCravateClassique> {
    ArrayList<ProduitCravateClassique> dataset;
    Activity act ;

    public ProduitAdapterCravateClassique(ArrayList<ProduitCravateClassique> DataModels, Activity act) {
    ...
    @Override
    public MyViewHolderCravateClassique onCreateViewHolder(ViewGroup parent, int viewType ) {
        View view= LayoutInflater.from( act ).inflate( R.layout.item_posts_cravate , parent,false );
        return new MyViewHolderCravateClassique( view);
    }

    @Override
    public void onBindViewHolder(MyViewHolderCravateClassique holder, int position) {
